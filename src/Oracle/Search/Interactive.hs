...
interactive :: (Monad m, MonadIO m) => [(String, Oracle m a)] -> SearchT m a -> m (Maybe a)
interactive oracles (SearchT k) = k >>= \case
  Fail   -> pure Nothing
  Done x -> pure (Just x)

  Choice cp@(ChoicePoint snapshot cs) -> do
    liftIO $ print (snapshot, fmap fst cs)
    for_ oracles $ \(name, oracle) -> do
      pi <- oracle cp
      liftIO . putStrLn $ "  - " ++ name ++ ": " ++ show pi
    user <- liftIO getLine
    let i = (read user :: Int)

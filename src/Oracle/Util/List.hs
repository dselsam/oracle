...
first f (x:xs) = case f x of
  Nothing -> Oracle.Util.List.first f xs
  Just y -> Just y
...
reduce op key (x0:xs) = foldl (\acc x -> op acc (key x)) (key x0) xs
...
split _ [] = []
split n l
  | n > 0 = (take n l) : (Oracle.Util.List.split n (drop n l))
